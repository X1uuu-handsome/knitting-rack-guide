...
function settings(){return `${sectionTop('本机数据','进度设置','所有勾选只保存在这台设备的浏览器里。')}<div class="content-stack"><div class="setting-card"><strong>当前安装进度</strong><p>${state.completed.length} / 18步，已收货 ${state.received.length} 项，验收 ${state.inspection.length} 项。</p></div><div class="setting-card"><strong>重置安装进度</strong><p>会清除当前步骤、已完成步骤、零件收货和验收勾选。</p><button class="danger-button" type="button" id="resetButton">重置全部进度</button></div><button class="subtle-button" type="button" data-go="home">返回首页</button></div>`;}
...
if('serviceWorker' in navigator && location.protocol.startsWith('http'))navigator.serviceWorker.register('./sw.js').catch(()=>showToast('离线缓存暂时未启用。'));
